    ...
    {showDiagnose && <div className="modal-backdrop" onMouseDown={() => { if (!diagnosing) { setShowDiagnose(false); setDiagnosis(undefined) } }}><div className="modal modal-wide" onMouseDown={(e) => e.stopPropagation()}>
      ...
      {diagnosis && <div className="diagnosis">
        ...
        {diagnosis.tweet && <dl className="estimate-grid">
          ...
          {diagnosis.mention_feed && <div><dt>Mention feed of @{diagnosis.mention_feed.target}</dt><dd><span className={`status ${diagnosis.mention_feed.found ? 'complete' : 'failed'}`}><i />{diagnosis.mention_feed.found ? 'returns it' : 'does not return it'}</span><small>{diagnosis.mention_feed.returned} mentions read around its date · cap {diagnosis.mention_feed.cap_pages} pages</small></dd></div>}
          ...
          {diagnosis.author_search && <div><dt>Search for the author's posts</dt><dd><span className={`status ${diagnosis.author_search.found ? 'complete' : 'failed'}`}><i />{diagnosis.author_search.found ? 'finds it' : 'does not find it'}</span><small>{diagnosis.author_search.returned} of their posts returned around its date</small></dd></div>}
