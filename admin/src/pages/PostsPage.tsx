...
export default function PostsPage({ session }: { session: Session }) {
  ...
  return <div className="page">
    ...
    <section className="posts-grid">
      {visible.map((post) => <article className={`post-card ${post.active ? '' : 'inactive'}`} key={post.tweet_id}><div className="post-top"><p className="post-handle">@{post.source_handle}</p><span className={`status ${post.active ? 'complete' : 'failed'}`}><i />{post.active ? 'Tracking' : 'Paused'}</span></div><h3>{post.post_created_at ? `Posted ${formatDate(post.post_created_at)}` : 'Publish date unknown'}</h3><dl><div><dt>Origin</dt><dd>{post.origin}</dd></div><div><dt>Last checked</dt><dd>{formatDate(post.last_checked_at)}</dd></div><div><dt>Post ID</dt><dd className="mono">{post.tweet_id}</dd></div></dl><div className="post-actions"><a href={post.url} target="_blank">Open on X <ExternalLink size={14} /></a><button className="icon-button" onClick={() => (post.active ? setConfirmPause(post) : toggle(post))} title={post.active ? 'Pause tracking' : 'Resume tracking'} aria-label={post.active ? `Pause tracking post ${post.tweet_id}` : `Resume tracking post ${post.tweet_id}`}>{post.active ? <ToggleRight /> : <ToggleLeft />}</button></div></article>)}
